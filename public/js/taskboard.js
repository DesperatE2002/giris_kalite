...
const TaskBoard = {
      ...
      <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
          ...
          <div class="flex gap-2 flex-wrap">
            <div class="bg-yellow-500/10 border border-yellow-500/20 rounded-xl px-4 py-2 text-center min-w-[80px]">
            ...
            <div class="bg-blue-500/10 border border-blue-500/20 rounded-xl px-4 py-2 text-center min-w-[80px]">
            ...
            <div class="bg-orange-500/10 border border-orange-500/20 rounded-xl px-4 py-2 text-center min-w-[80px]">
            ...
            <div class="bg-purple-500/10 border border-purple-500/20 rounded-xl px-4 py-2 text-center min-w-[80px]">
            ...
            <div class="bg-green-500/10 border border-green-500/20 rounded-xl px-4 py-2 text-center min-w-[80px]">
            ...
            return `
              <div class="glass-card rounded-2xl overflow-hidden">
                ...
                <div class="p-4 space-y-3">
                  ...
                  ${doneTasks.length > 0 ? `
                    <details class="group">
                      <summary class="cursor-pointer text-xs text-gray-400 hover:text-gray-300 font-medium py-1">
  ...
  renderTask(a) {
    ...
    if (a.deadline && a.status !== 'completed') {
      ...
      const cls = isLate ? 'bg-red-500/20 text-red-400' : isClose ? 'bg-orange-500/20 text-orange-400' : 'bg-blue-500/10 text-blue-400';
    ...
    // Durum renkleri
    const statusCfg = {
      pending:   { bg: 'bg-yellow-500/10 border-yellow-500/20', dot: 'bg-yellow-400', label: 'Bekliyor' },
      active:    { bg: 'bg-blue-500/10 border-blue-400/30 ring-2 ring-blue-500/20', dot: 'bg-blue-500 animate-pulse', label: 'Çalışılıyor' },
      paused:    { bg: 'bg-purple-500/10 border-purple-400/30', dot: 'bg-purple-500', label: 'Pause' },
      blocked:   { bg: 'bg-orange-500/10 border-orange-400/30', dot: 'bg-orange-500', label: 'Bloke' },
      completed: { bg: 'bg-green-500/10 border-green-500/20', dot: 'bg-green-500', label: 'Tamamlandı' }
    };
    ...
    if (a.status === 'pending') {
      ...
      buttons = `
        <span class="bg-indigo-500/20 text-indigo-400 font-bold px-4 py-2 rounded-xl text-sm">
      ...
      <div class="border rounded-xl p-4 ${st.bg} flex flex-col sm:flex-row sm:items-center gap-3 transition-all">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-1">
            ...
            <h4 class="font-bold text-white text-base truncate">${a.title}</h4>
            <span class="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-gray-400 font-medium flex-shrink-0">${st.label}</span>
          </div>
          ${a.description ? `<p class="text-sm text-gray-400 ml-5 mb-1 line-clamp-1">${a.description}</p>` : ''}
          <div class="flex flex-wrap items-center gap-2 ml-5 text-xs">
            ...
            ${durationText ? `<span class="text-gray-400"><i class="fas fa-stopwatch mr-1 text-indigo-400"></i>${durationText}</span>` : ''}
            ${deadlineHtml}
          ...
          ${a.notes ? `<div class="text-xs text-gray-400 ml-5 mt-1 bg-white/[0.06] rounded px-2 py-1"><i class="fas fa-sticky-note mr-1 text-yellow-500"></i>${a.notes}</div>` : ''}
        </div>
    ...
    modal.innerHTML = `
      <div class="bg-gray-900 border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div class="mb-4">
          ...
          <textarea id="tb_completeNote" rows="3" class="w-full px-4 py-3 border border-white/10 rounded-xl focus:border-green-500 focus:outline-none text-base bg-gray-800/50 text-white" placeholder="Not ekleyin..."></textarea>
        </div>
        <div class="flex gap-3">
          ...
          <button onclick="this.closest('.fixed').remove()" class="px-6 py-3 border border-white/10 rounded-xl text-gray-400 hover:bg-white/5 font-semibold">İptal</button>
        </div>
      </div>
    `;
    ...
    modal.innerHTML = `
      <div class="bg-gray-900 border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div class="mb-4">
          ...
          <textarea id="tb_blockReason" rows="3" class="w-full px-4 py-3 border border-white/10 rounded-xl focus:border-orange-500 focus:outline-none text-base bg-gray-800/50 text-white" placeholder="Neden bloke edildi?"></textarea>
        </div>
        <div class="flex gap-3">
